...
export function Canvas({ readOnly = false }: { readOnly?: boolean } = {}) {
  ...
  const saveCanvas = useCallback(() => {
    ...
    api.canvas.save(fileId, {
      objects:    store.objects as Record<string, unknown>,
  ...
  useEffect(() => {
    ...
    return () => {
      unsub();
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
      // Final save on unmount — read fileId directly here; saveCanvas() can
      // no-op if activeFileId was already cleared by a parent's cleanup.
      const fileId = useWorkspaceStore.getState().activeFileId;
      if (fileId) {
